...
import { Search, Sun, Moon, Home, Users, ShieldAlert, BarChart3, FileText, Activity } from 'lucide-react';
...
const adminMenuItems = [
  { label: 'Admin Dashboard', href: '/portal/admin/dashboard', icon: <Home size={18} /> },
  { label: 'User Management', href: '/portal/admin/users', icon: <Users size={18} /> },
  { label: 'Object Moderation', href: '/portal/admin/moderation', icon: <ShieldAlert size={18} /> },
  { label: 'Analytics', href: '/portal/admin/analytics', icon: <BarChart3 size={18} /> },
  { label: 'Reports', href: '/portal/admin/reports', icon: <FileText size={18} /> },
  { label: 'System Health', href: '/portal/admin/health', icon: <Activity size={18} /> },
];
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 border-b border-[#B0BEC5]/30 dark:border-[#263238]/30 flex items-center justify-between px-8 bg-white/70 dark:bg-[#162033]/20 backdrop-blur-md z-20 shrink-0">
          ...
          <div className="flex items-center gap-6">
            <span className="font-display font-bold text-sm tracking-tight text-red-600 dark:text-red-500">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 select-none">
              ...
              <div className="hidden lg:flex flex-col text-left">
                <span className="text-xs font-bold text-[#1F2937] dark:text-[#F8FAFC]">{user?.name || 'Administrator'}</span>
                <span className="text-[10px] text-red-500 font-bold uppercase">{user?.role || 'Admin'}</span>
              </div>
            </div>
          </div>
        </header>

        {/* Admin Workspace Outlet */}
